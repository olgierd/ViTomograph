...
public class Tomograph {
	...
	public Tomograph(BufferedImage img, int numberOfDetectors,
			double beamWidth, int distance, boolean withDSP) {
		...
		outputImage = new BufferedImage(img.getWidth(), img.getHeight(),
				BufferedImage.TYPE_INT_RGB);
		outputImageRaw = new BufferedImage(numberOfDetectors, 361,
				BufferedImage.TYPE_INT_RGB);

		currentReadoutImage = new BufferedImage(numberOfDetectors, 260,
				BufferedImage.TYPE_INT_RGB);
		clearImage = new BufferedImage(numberOfDetectors, 260,
				BufferedImage.TYPE_INT_RGB);
	...
	public void getLine() {
		...
		for (i = 0; i < tab.length; i++) {
			outputImageRaw.setRGB(i, rotationAngle, tab[i] + (tab[i] << 8)
					+ (tab[i] << 16));
		}
	...
	public void calculateSingleDetector(Detector d) {
		...
		for (Point2D.Double p : points) {
			...
			if (i > 0 && j > 0 && i < img.getWidth() && j < img.getHeight()) {
				...
				if (i % 1 == 0 && (int) i < img.getWidth() - 1
						&& (int) j < img.getHeight() - 1) {
					value = (img.getRGB((int) i, (int) j)) * (1 - (j % 1));
					value += (img.getRGB((int) i, (int) j + 1)) * (j % 1);
				}

				if (j % 1 == 0 && (int) i < img.getWidth() - 1
						&& (int) j < img.getHeight() - 1) {
	...
	public void drawToResultTable(Point from, Point to, double color) {
		...
		for (Point2D.Double p : points) {
			...
			if (i > 0 && j > 0 && i < outputImage.getWidth()
					&& j < outputImage.getHeight()) {
	...
	public void doFullCapture() {
		...
		for (int i = 0; i < 361; i += 1) {
			...
			System.out.println((int) (100 * i / 360) + "% # (" + i
					+ "/360) # Processing time: " + (loopend - loopstart)
					+ "ms #  ETA: " + (loopend - loopstart) * (360 - i)
					/ 1000.0 + " seconds");
		}
